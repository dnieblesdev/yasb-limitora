...
| D01a3a2a | Durable marker IO | Bounded ≤256 same-handle read after SetFilePointerEx reset; loop on actual WriteFile progress with invalid/zero-progress refusal; FlushFileBuffers only after complete write; sanitized IO errors; fake and real Windows proof; no identity re-open/delete or Guard/reclaim/deadline policy | D01a3a1 |
| D01a3a2b | Identity re-open and disposition delete | OPEN_EXISTING on exact fixed leaf; verify exact final path, non-reparse regular file, and volume/file identity; close probes; disposition-delete only the original DELETE-capable handle; prove alias refusal, successor safety, deletion, and no residue; no marker IO change or Guard/reclaim/deadline policy | D01a3a2a |
| D01a3b | Fallback policy and integration | Private typed ConfigLease unifies mutex/marker ownership; one shared DeadlineContext for D01a3b and D01b; fallback only on guard_acquisition_failed; retry/contention; decode/owner predicate; conservative busy rules; reclaim only for ProcessTokenMissing or returned token mismatch; cleanup outcome maps config-lock-release-failed | D01a3a2b |
...
>
> **D01a3a2 budget raised.** The D01a3a2 candidate (evidence `sha256:607410ff0e6ea1d29095d93dff11e9f047cd817e201add052bdd8054f84963a9`) passed focused 47, native 11, full 900, and Ruff but exceeded the ≤280 internal budget at 341 lines. It supplied no passing evidence. The maintainer initially raised the cohesive unit to ≤350 lines because the candidate remained below the canonical 400-line PR boundary.
>
> **D01a3a2 split after semantic remediation.** Independent evidence `sha256:32663eaf69165ee330872ebc8b6f6ab4f05fe8fd8355de579d8ed91d498bf6fe` rejected D01a3a2 because `WriteFile` progress was ignored, same-handle reads did not reset the pointer, and identity re-open did not enforce the exact fixed final leaf. Corrected evidence `sha256:a645ccce8af14a3d9cc511f9f5ea1a3efb22258f5ac648658dfe89e3dd5cb501` fixed those semantics and passed focused 45 but measured 359/350 lines, so it also supplies no passing evidence. The maintainer authorized D01a3a2a (durable marker IO, ≤280 lines) followed by D01a3a2b (fixed-leaf identity re-open and disposition delete, ≤280 lines). D01a3b now depends on D01a3a2b; D01b remains dependent on D01a3b. Neither child inherits passing evidence; both remain unchecked. No source, tests, progress, RDD, or delivery is claimed.
...
```text
Config:
  preserved baseline → D01a1 → D01a2a → D01a2b → D01a3a1 → D01a3a2a → D01a3a2b → D01a3b → D01b → D02 → {D03, D04}
