...
- Final per-slice accounting for the seven physical rename work units is **20,254 native/no-rename lines** and **1,470 Git rename-aware lines**. Review notes must record both numbers; each exception budget is based on its native/no-rename slice.
- Exact delete+add drivers are: normative document rename **3,779**, projection source plus projection-test rename **≤2,800** (including legacy projection replacement and import edits), cache source plus cache-test rename **≤4,500**, worker source plus worker-test rename **≤2,700**, guard/deadline/path sources plus guard/deadline/path/file-read tests **≤3,800**, spec-test rename **1,487**, and schema rename **1,458** lines.
...
| `size:exception` 6: spec-test rename | Mechanical spec-test filename/path rename only; **bounded exception: 1,487 no-rename lines**; Git rename-aware reference recorded | Start after exception 5; finish with collected spec tests and references, with no behavior edits. | Prove rename-only diff; collect and run spec-related tests. | Revert only spec-test rename. |
| `size:exception` 7: schema rename | Mechanical schema filename/path rename only; **bounded exception: 1,458 no-rename lines**; Git rename-aware reference recorded | Start after exception 6; finish with all schema references updated and no contract changes. | Prove rename-only diff; schema/link validation and dependent collection. | Revert only schema rename. |
...
- [x] Add the planning correction first on `feature/137-json-budget-repartition`; do not use `13f63d6`.
...
- [x] Record the seven bounded rename-only exception budgets (normative doc, projection, cache, worker, guard/deadline/path, spec tests, schema) and Git rename-aware guidance; do not hide rename work in semantic slices.
...
- [x] GREEN: implement contract/cache/CLI/runtime semantics in dependency order; migrate consumers before any provider rename or deletion.
- [x] TRIANGULATE: run focused success, failure, not-run, malformed, ordering, size, redaction, LF, stream/exit, precedence, deadline, stale-cache, import, process, guard, jobs, and spawn checks; require collection to remain clean.
- [x] REFACTOR: remove duplicate `project_bytes` and any behavior-slice lint debt introduced by the work; distinguish pre-existing named worker/test lint debt.
...
- [x] RED/GREEN/TRIANGULATE/REFACTOR each remaining bounded legacy deletion child at ≤400 no-rename lines.
...
- [x] `size:exception` 3: mechanically rename the cache source/tests, remove `V2QuotaCache`, and normalize active cache imports/helpers/labels; preserve behavior and identities.
- [x] `size:exception` 4: mechanically rename the worker source/tests and normalize direct CLI/runtime/native/platform consumers; preserve process behavior and identities.
- [x] `size:exception` 5: mechanically rename the guard/deadline/path sources and guard/deadline/path/file-read tests; normalize active imports, symbols, and CLI/path consumers while preserving lifecycle, deadline, path, file-read behavior, and identities.
- [x] `size:exception` 6: mechanically rename `tests/test_json_v2_spec.py` to `tests/test_json_spec.py`; preserve test bytes, names, assertions, schema path, and contract semantics.
- [x] `size:exception` 7: mechanically rename `docs/specifications/json-v2.schema.json` to `docs/specifications/json-output.schema.json`; preserve schema bytes except the `$id` path and normalize active references.
- [x] After all consumers migrate, execute exactly the seven reviewable rename exceptions (normative doc, projection source/tests, cache source/tests, worker source/tests, guard/deadline/path sources/tests, spec tests, schema), each with its stated no-rename budget.
- [x] For every exception, prove rename-only/no semantic edits, record native/no-rename and Git rename-aware counts, and run collection plus its scoped checks; combined source/test slices are permitted only when branch collection remains valid.
- [x] Search active source/tests/examples/docs/packaging for selectors, v1 artifacts, deleted names, and active versioned implementation names; exclude only designated historical text and immutable persisted identities.
- [x] Record per-branch test results, strict-marker results, native Windows status, collection status, rollback boundary, and clean final diff accounting before delivery.
