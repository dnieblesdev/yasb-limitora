...
explicitly rejected. This Windows-only boundary returns exit code `2` outside
Windows, writes exactly `yasb-limitora: unsupported_platform\n` to stderr, and
writes zero stdout bytes; Linux, macOS, and WSL are not supported runtimes.
...
| Existing YASB-side runtime | The current JSON document is the sole supported output, with fixed `codex`, then `opencode_go` provider order and no root `version` field. |
| Existing Codex IPC | Control payloads are bounded at 16 KiB and response payloads at 64 KiB; the current boundary must not silently enlarge those limits. |
...
current requirement, and must not be recreated.
    ...
    1. Automated native Windows proof covers the `yasb-limitora` CLI and current JSON
    contract.
    2. The maintainer accepted the real YASB CustomWidget manually on YASB v2.0.6.
    No automated YASB rendering or external YASB E2E is claimed.
