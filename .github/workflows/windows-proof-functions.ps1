...
function Get-R10JUnitDiagnostic([string[]]$Arguments) {
  try {
    ...
    $r = [xml](Get-Content -LiteralPath $paths[0] -Raw -ErrorAction Stop); $c = @(@($r.testsuites.testsuite) + @($r.testsuite) | % { $_.testcase } | ? { $null -ne $_.failure -or $null -ne $_.error } | select -First 1); if ($null -eq $c) { return "diagnostics withheld" }; $n = if ($null -ne $c.failure) { $c.failure } else { $c.error }; $raw = "$($n.message) $($n.InnerText)"; $lm = [regex]::Match($raw, '(?i)(?:(?:[A-Za-z]:[\\/]|/|(?:[^\\/:<>"'']+[\\/])+)[^:\r\n]+:(?<line>[0-9]{1,6})(?::[0-9]+)?|File\s+["''](?:[A-Za-z]:[\\/]|/)[^"'']+["''],\s+line\s+(?<line>[0-9]{1,6})(?:,\s+in\b|\b))'); if (!$lm.Success) { return "diagnostics withheld" }
    $s = { param($v) $v = [regex]::Replace([string]$v, '(?i)\b(?:password|passwd|credential|api[ _-]?key|authorization|cookie|token|secret)\b[ \t]*[:=][ \t]*(?:"[^"\r\n]*"|''[^''\r\n]*''|[^,;\r\n]*)', '[REDACTED]'); [regex]::Replace([regex]::Replace($v, '(?i)[A-Za-z]:[\\/][^\r\n"''<>;,]*(?=["'']?(?::[0-9]{1,6}(?::[0-9]+)?(?:\b|$)|[;,])|["''],\s+line\s+[0-9]{1,6})|(?<![\w])/(?:[^/\r\n"''<>;,]+/)+[^\r\n"''<>;,]*(?=["'']?(?::[0-9]{1,6}(?::[0-9]+)?(?:\b|$)|[;,])|["''],\s+line\s+[0-9]{1,6})', '[PATH]'), '\s+', ' ').Trim() }; $d = "R10 diagnostic: test=$(&$s $c.name); line=$($lm.Groups.line.Value); message=$(&$s $raw)"; if ($d.Length -gt 240) { $d = $d.Substring(0, 240) }; return $d
