...
function Get-R10JUnitDiagnostic([string[]]$Arguments) {
  try {
    ...
    $r = [xml](Get-Content -LiteralPath $path -Raw -ErrorAction Stop); $stage = "candidate"; $c = @(@($r.testsuites.testsuite) + @($r.testsuite) | % { $_.testcase } | ? { $null -ne $_.failure -or $null -ne $_.error } | select -First 1); if ($null -eq $c) { return & $withheld $stage }; $stage = "node"; $n = if ($null -ne $c.failure) { $c.failure } else { $c.error }; $raw = "$($n.message) $($n.InnerText)"; $stage = "match"; $lm = [regex]::Match($raw, '(?i)(?:(?:[A-Za-z]:[\\/]|/|(?:[^\\/:<>"'']+[\\/])+)[^:\r\n]+:(?<line>[0-9]{1,6})(?::[0-9]+)?|File\s+["''](?:[A-Za-z]:[\\/]|/)[^"'']+["''],\s+line\s+(?<line>[0-9]{1,6})(?:,\s+in\b|\b))'); if (!$lm.Success) { return & $withheld $stage }
