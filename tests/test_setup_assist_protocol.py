...
# --- Bounded reason channel (assistant side) ---

def test_reason_env_constant_exists():
    """The assistant defines the reason environment variable constant."""
    assert hasattr(sa, "_REASON_ENV")
    assert sa._REASON_ENV == "_YASB_SETUP_ASSIST_REASON"


def test_reason_written_for_non_ok_operation(tmp_path):
    """When operations return non-ok, the reason file gets the first non-ok reason."""
    reason_file = tmp_path / "reason.txt"
    raw = request_bytes([{"operation": "state-cleanup", "consent": "YES"}])
    la = tmp_path / "la"
    la.mkdir()
    # state-cleanup with no prior ownership will refuse; the reason channel
    # must capture whatever the first non-ok reason code is.
    exit_code = sa._run_setup_assist(
        {sa._REQUEST_ENV: raw.decode(), sa._REASON_ENV: str(reason_file)},
        local_appdata=str(la),
    )
    assert exit_code == 1
    assert reason_file.exists()
    reason_text = reason_file.read_text(encoding="utf-8")
    # The reason must be a non-empty bounded code (not a path, not user content)
    assert len(reason_text) > 0
    assert len(reason_text) <= 256
    assert "\\" not in reason_text and "/" not in reason_text


def test_reason_written_for_validation_failure(tmp_path):
    """When request validation fails, the reason file gets the violation code."""
    reason_file = tmp_path / "reason.txt"
    raw = request_bytes([{"operation": "unknown-op"}])
    exit_code = sa._run_setup_assist(
        {sa._REQUEST_ENV: raw.decode(), sa._REASON_ENV: str(reason_file)},
    ...
    assert reason_file.exists()
    reason_text = reason_file.read_text(encoding="utf-8")
    assert reason_text == "unknown-operation"


def test_reason_not_written_when_env_not_set(tmp_path):
    """No reason file is written when the env var is not set."""
    raw = request_bytes([{"operation": "unknown-op"}])
    exit_code = sa._run_setup_assist({sa._REQUEST_ENV: raw.decode()})
    assert exit_code == 1
    # No reason file should be created
    assert not (tmp_path / "reason.txt").exists()


def test_reason_not_written_when_env_not_absolute(tmp_path):
    """No reason file is written when the env var is not an absolute path."""
    raw = request_bytes([{"operation": "unknown-op"}])
    exit_code = sa._run_setup_assist(
        {sa._REQUEST_ENV: raw.decode(), sa._REASON_ENV: "relative/path.txt"},
    )
    assert exit_code == 1
    # No reason file should be created
    assert not (tmp_path / "relative" / "path.txt").exists()


def test_reason_write_failure_does_not_change_exit_code(tmp_path):
    """If the reason file can't be written, the exit code doesn't change."""
    # Use an invalid path that can't be written
    reason_file = tmp_path / "nonexistent" / "dir" / "reason.txt"
    raw = request_bytes([{"operation": "unknown-op"}])
    exit_code = sa._run_setup_assist(
        {sa._REQUEST_ENV: raw.decode(), sa._REASON_ENV: str(reason_file)},
    )
    # Exit code is still 1 (validation failure), not changed by write failure
    assert exit_code == 1
    # Reason file was not created (parent dir doesn't exist)
    assert not reason_file.exists()


def test_reason_bounded_to_256_bytes(tmp_path):
    """The reason written to the file is bounded to 256 bytes."""
    reason_file = tmp_path / "reason.txt"
    # Create a scenario that produces a long reason (use a long violation code)
    # Actually, the reason codes are short, so we test the bounding mechanism
    # by checking that the _write_reason function truncates long reasons
    sa._write_reason({sa._REASON_ENV: str(reason_file)}, "x" * 500)
    assert reason_file.exists()
    reason_text = reason_file.read_text(encoding="utf-8")
    assert len(reason_text) <= 256
    assert reason_text == "x" * 256
