...
def test_r10_junit_diagnostic_integration_contract(tmp_path: Path) -> None:
    shell = next(shutil.which(name) for name in ("pwsh", "powershell") if shutil.which(name)); wrapper = Path(__file__).parents[1] / ".github/workflows/windows-proof-functions.ps1"; cases = (("r10-admission.xml", r'File "C:\Users\Jane Doe\repo\test.py", line 12, in test_name password=SECRET; token=SECOND', 'R10 diagnostic: test=windows; line=12; message=File "[PATH]", line 12, in test_name [REDACTED]; [REDACTED]'), ("r10-admission.xml", r"File '/home/Jane Doe/repo/test.py', line 7, in test_name api_key='VALUE'", "R10 diagnostic: test=posix; line=7; message=File '[PATH]', line 7, in test_name [REDACTED]"), ("r10-admission.xml", r"tests/test_parser.py:23: message credential=VALUE", "R10 diagnostic: test=relative; line=23; message=tests/test_parser.py:23: message [REDACTED]"), ("r10-admission.xml", "tests/test_parser.py:99: " + "x" * 300, ("R10 diagnostic: test=cap; line=99; message=tests/test_parser.py:99: " + "x" * 300)[:240]), ("r10-admission.xml", "MALFORMED", "diagnostics withheld"), ("r10-yasb-experience.xml", None, "diagnostics withheld")); names = ("windows", "posix", "relative", "cap", "malformed", "missing"); [((tmp_path / name).unlink(missing_ok=True) if message is None else (tmp_path / name).write_text("<testsuite><testcase name=\"" + names[index] + "\"><failure message=\"" + ("<testsuite>" if message == "MALFORMED" else message.replace("&", "&amp;").replace('"', "&quot;").replace("'", "&apos;")) + "\"/></testcase></testsuite>", encoding="utf-8")) for index, (name, message, _) in enumerate(cases)]; outputs = [subprocess.run([shell, "-NoProfile", "-NonInteractive", "-Command", f". '{wrapper}'; Get-R10JUnitDiagnostic @('--junitxml={name}')"], cwd=tmp_path, capture_output=True, text=True) for name, _, _ in cases]; assert all(result.returncode == 0 for result in outputs) and [[result.stdout.strip()] for result in outputs] == [[expected] for _, _, expected in cases]
